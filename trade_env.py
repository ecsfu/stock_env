...
class StockTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        state_space, #状态空间
        action_space, #动作空间
        tech_indicator_list,  #技术指标
        turbulence_threshold=None,  #动荡指数门限值（衡量系统性风险）
        risk_indicator_col="turbulence", #风险指标
        make_plots=False,   #画图
        ...
        self.data = self.df.loc[self.day, :] #数据取起始日
        self.terminal = False
        self.make_plots = make_plots
        self.print_verbosity = print_verbosity
        self.turbulence_threshold = turbulence_threshold
        self.risk_indicator_col = risk_indicator_col
        self.initial = initial  #初始化状态？
    ...
    def step(self, actions):
        self.terminal = self.day >= len(self.df.index.unique()) - 1 #走到最后一步
        if self.terminal:
            # print(f"Episode: {self.episode}")
            if self.make_plots:  #绘图
                self._make_plot()
            ...
            if self.episode % self.print_verbosity == 0:
                ...
                if df_total_value["daily_return"].std() != 0:
                    print(f"Sharpe: {sharpe:0.3f}")   #有波动则可算夏普率
                print("=================================")
            ...
            actions = actions * self.hmax  # actions initially is scaled between 0 to 1  #0-100股，交易量比较小哎
            actions = actions.astype(
                int
            )  # convert into integer because we can't by fraction of shares
            if self.turbulence_threshold is not None:
                if self.turbulence >= self.turbulence_threshold:
                    actions = np.array([-self.hmax] * self.stock_dim)  #清仓跑路
        ...
        return self.state, self.reward, self.terminal, {}  #返回状态、奖励、是否结束，空

    def reset(self):
        # initiate state
        self.state = self._initiate_state()

        if self.initial:   #初始化则资产等于环境传入的现金和股份，否则等于前面状态的现金和股份，代表接着完还是重开
    ...
    def _initiate_state(self):
        if self.initial:
            ...
            if len(self.df.tic.unique()) > 1:
                # for multiple stock
                state = (
                    [self.initial_amount]  #现金
                    + self.data.close.values.tolist()  #股票价格
                    + self.initial_list[1:]  #股份
                    + sum(
                        [
        ...
        if len(self.df.tic.unique()) > 1:
            ...
            state = (
                ...
                + sum(
                    ...
                    [],
                )  #sum将2维降为1维，包含了数据框中现金，所有的价格、股份、所有技术指标的值
            )
